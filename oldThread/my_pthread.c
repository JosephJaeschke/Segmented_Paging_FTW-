...
//	printf("--create\n");
	if(ptinit==0)
	{
		...
		queue=(tcb**)malloc(PRIORITY_LEVELS*sizeof(tcb));
		terminating=(tcb*)malloc(sizeof(tcb));
		int i=0;
		...
		tcb* maint=(tcb*)malloc(sizeof(tcb));
		maint->state=0;
		maint->tid=idCounter++;
...
//		makecontext(&ctx_clean,clean,0);
		...
		ctx_sched.uc_link=0;
		ctx_sched.uc_stack.ss_sp=(void*)malloc(MAX_STACK);
...
//		printf("--timer set\n");
	...
	ctx_func.uc_stack.ss_sp=(void*)malloc(MAX_STACK);
